...
internal_database_basis::internal_database_basis(path const& db_dir, uint32_t reorg_pool_limit, uint64_t db_max_size, bool safe_mode)
    : db_dir_(db_dir)
...
internal_database_basis::~internal_database_basis() {
    close();
}

#if ! defined(KTH_DB_READONLY)

bool internal_database_basis::create() {
    std::error_code ec;
...
bool internal_database_basis::create_db_mode_property() {
...
bool internal_database_basis::open() {
...
bool internal_database_basis::open_internal() {
...
bool internal_database_basis::verify_db_mode_property() const {
...
bool internal_database_basis::close() {
    if (db_opened_) {
...
result_code internal_database_basis::push_genesis(domain::chain::block const& block) {
...
utxo_entry internal_database_basis::get_utxo(domain::chain::output_point const& point, KTH_DB_txn* db_txn) const {
...
utxo_entry internal_database_basis::get_utxo(domain::chain::output_point const& point) const {
...
result_code internal_database_basis::get_last_height(uint32_t& out_height) const {
    KTH_DB_txn* db_txn;
    auto res = kth_db_txn_begin(env_, NULL, KTH_DB_RDONLY, &db_txn);
    if (res != KTH_DB_SUCCESS) {
        return result_code::other;
...
std::pair<domain::chain::header, uint32_t> internal_database_basis::get_header(hash_digest const& hash) const {
...
domain::chain::header internal_database_basis::get_header(uint32_t height) const {
    KTH_DB_txn* db_txn;
...
domain::chain::header::list internal_database_basis::get_headers(uint32_t from, uint32_t to) const {
...
result_code internal_database_basis::pop_block(domain::chain::block& out_block) {
    uint32_t height;
    ...
    out_block = get_block_reorg(height, dbi_reorg_block_, env_);
...
result_code internal_database_basis::prune() {
    //TODO: (Mario) add overload with tx
    ...
    res = get_first_reorg_block_height(first_height, dbi_reorg_block_, env_);
    if (res == result_code::db_empty) return result_code::no_data_to_prune;
    if (res != result_code::success) return res;
    ...
    res = prune_reorg_block(amount_to_delete, db_txn, dbi_reorg_block_);
    ...
    res = prune_reorg_index(remove_until, db_txn, dbi_reorg_pool_, dbi_reorg_index_);
    if (res != result_code::success) {
        kth_db_txn_abort(db_txn);
        return res;
    }
...
result_code internal_database_basis::insert_reorg_into_pool(utxo_pool_t& pool, KTH_DB_val key_point, KTH_DB_txn* db_txn) const {
...
std::pair<result_code, utxo_pool_t> internal_database_basis::get_utxo_pool_from(uint32_t from, uint32_t to) const {
    // precondition: from <= to
    utxo_pool_t pool;
...
result_code internal_database_basis::push_transaction_unconfirmed(domain::chain::transaction const& tx, uint32_t height) {
...
// Private functions
// ------------------------------------------------------------------------------------------------------
size_t internal_database_basis::get_db_page_size() const {
    return boost::interprocess::mapped_region::get_page_size();
}

size_t internal_database_basis::adjust_db_size(size_t size) const {
...
bool internal_database_basis::create_and_open_environment() {
...
#else
    std::cout << "safe_mode_: " << safe_mode_ << "\n";
    if ( ! safe_mode_) {
        // Monero:    mdb_flags |= MDB_NOSYNC | MDB_WRITEMAP | MDB_MAPASYNC;
        // Old Knuth: mdb_flags |= KTH_DB_WRITEMAP | KTH_DB_MAPASYNC;
        mdb_flags |= KTH_DB_NOSYNC | KTH_DB_WRITEMAP | KTH_DB_MAPASYNC;
        // mdb_flags |= MDB_NOSYNC | MDB_WRITEMAP | MDB_MAPASYNC;
    }
#endif
    std::cout << "mdb_flags: " << mdb_flags << "\n";
...
/*
bool internal_database_basis::set_fast_flags_environment(bool enabled) {
...
bool internal_database_basis::open_databases() {
    KTH_DB_txn* db_txn;
...
result_code internal_database_basis::remove_inputs(hash_digest const& tx_id, uint32_t height, domain::chain::input::list const& inputs, bool insert_reorg, KTH_DB_txn* db_txn) {
    uint32_t pos = 0;
    ...
    result_code res;
        ...
        res = remove_utxo(height, prevout, insert_reorg, db_txn, dbi_utxo_, dbi_reorg_pool_,  dbi_reorg_index_);
        if (res != result_code::success) {
            return res;
        }
...
result_code internal_database_basis::insert_outputs(hash_digest const& tx_id, uint32_t height, domain::chain::output::list const& outputs, data_chunk const& fixed_data, KTH_DB_txn* db_txn) {
    uint32_t pos = 0;
...
result_code internal_database_basis::insert_outputs_error_treatment(uint32_t height, data_chunk const& fixed_data, hash_digest const& txid, domain::chain::output::list const& outputs, KTH_DB_txn* db_txn) {
...
void internal_database_basis::print_stats(KTH_DB_txn* db_txn) {
    ...
    if (ret == KTH_DB_SUCCESS) {
        std::cout << "headers count: " << db_stats.ms_entries << std::endl;
    }
    ret = mdb_stat(db_txn, dbi_block_header_by_hash_, &db_stats);
    if (ret == KTH_DB_SUCCESS) {
        std::cout << "headers by hash count: " << db_stats.ms_entries << std::endl;
    }
    ret = mdb_stat(db_txn, dbi_utxo_, &db_stats);
    if (ret == KTH_DB_SUCCESS) {
        std::cout << "UTXO Set count: " << db_stats.ms_entries << std::endl;
    }
    ret = mdb_stat(db_txn, dbi_reorg_pool_, &db_stats);
    if (ret == KTH_DB_SUCCESS) {
        std::cout << "Reorg Pool count: " << db_stats.ms_entries << std::endl;
    }
    ret = mdb_stat(db_txn, dbi_reorg_index_, &db_stats);
    if (ret == KTH_DB_SUCCESS) {
        std::cout << "Reorg Pool Index count: " << db_stats.ms_entries << std::endl;
    }
    ret = mdb_stat(db_txn, dbi_reorg_index_, &db_stats);
    if (ret == KTH_DB_SUCCESS) {
...
result_code internal_database_basis::push_block(domain::chain::block const& block, uint32_t height, uint32_t median_time_past, bool insert_reorg, KTH_DB_txn* db_txn) {
    ...
    // std::cout << "insert_utxo() -- commented code that kth_db_put" << std::endl;
    // std::cout << "UTXO FULL" << std::endl;
    // std::cout << "UTXO NO remove inputs" << std::endl;
    // return result_code::success;
...
result_code internal_database_basis::push_genesis(domain::chain::block const& block, KTH_DB_txn* db_txn) {
...
result_code internal_database_basis::remove_outputs(hash_digest const& txid, domain::chain::output::list const& outputs, KTH_DB_txn* db_txn) {
    uint32_t pos = outputs.size() - 1;
    for (auto const& output: outputs) {
        domain::chain::output_point const point {txid, pos};
        auto res = remove_utxo(0, point, false, db_txn, dbi_utxo_, dbi_reorg_pool_, dbi_reorg_index_);
        if (res != result_code::success) {
            return res;
        }
        --pos;
...
result_code internal_database_basis::insert_inputs(domain::chain::input::list const& inputs, KTH_DB_txn* db_txn) {
    for (auto const& input: inputs) {
        auto const& point = input.previous_output();

        auto res = insert_output_from_reorg_and_remove(point, db_txn, dbi_utxo_, dbi_reorg_pool_);
        if (res != result_code::success) {
            return res;
        }
    }
...
result_code internal_database_basis::remove_block(domain::chain::block const& block, uint32_t height, KTH_DB_txn* db_txn) {
    //precondition: block.transactions().size() >= 1
    ...
    res = remove_block_reorg(height, db_txn, dbi_reorg_block_);
    if (res != result_code::success) {
        return res;
    }

    res = remove_reorg_index(height, db_txn, dbi_reorg_index_);
...
result_code internal_database_basis::remove_block(domain::chain::block const& block, uint32_t height) {
    KTH_DB_txn* db_txn;
    auto res0 = kth_db_txn_begin(env_, NULL, 0, &db_txn);
    if (res0 != KTH_DB_SUCCESS) {
        return result_code::other;
